...
class GridOption:
	def __init__(self, dataset_pathname, options):
		...
		if sys.platform != 'win32':
			self.svmtrain_pathname = os.path.join(dirname, '../julia/shellvalidation.jl')
...
class Worker(Thread):
	...
	def get_cmd(self,c,g,p):
		...
		cmdline += ' '
		cmdline += options.dataset_pathname
		cmdline += ' '
		cmdline += '{0}'.format(options.fold)
		cmdline += ' svm_type=EPSILON_SVR'
		if options.grid_with_c: 
			cmdline += ',C={0}'.format(c)
		if options.grid_with_g: 
			cmdline += ',gamma={0}'.format(g)
...
def set_for_parallel(num_w, ssh_w, telnet_w):
	global nr_local_worker
	global ssh_workers
	global telnet_workers
	nr_local_worker = num_w
	ssh_workers = ssh_w
	telnet_workers = telnet_w

def printw():
	print(nr_local_worker)
